...
// Umbrales conservadores para cuidar la cuenta de WhatsApp (editables acá).
const LIMITES = {
...
export default function TermometroEnvio({ compact = false }: { compact?: boolean }) {
  ...
  if (compact) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '0.78rem', color: 'var(--muted)' }}>
        ...
        <span style={{ color: 'var(--text)', fontWeight: 600 }}>WhatsApp hoy: {estado.hoy}</span>
    ...
    <div style={{
      ...
      {nivel !== 'verde' && (
        <div style={{ fontSize: '0.76rem', color: 'var(--muted)', borderTop: `1px solid ${color}33`, paddingTop: 6, marginTop: 2 }}>
          💡 Para no arriesgar la cuenta, contactá a los próximos por otro canal:{' '}
          <a href="/admin/instagram-dm" style={{ color: 'var(--accent)', fontWeight: 600, textDecoration: 'none' }}>Instagram</a>
          {' '}o email. Priorizá los que tengan Instagram cargado.
        </div>
      )}
    </div>
  )
